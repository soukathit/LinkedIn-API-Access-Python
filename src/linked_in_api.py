...
#linkedin_paramters_file_path ="C:\Soukath\Projects\Digital Marketing\Linked In API Access\bin\LinkedIn_Config_Parameters.json"
...
driver = webdriver.Firefox(firefox_options=options,executable_path="/root/LinkedIn_API_Access/bin/geckodriver")
#The Executable Path should point to the geckodriver path of the location where it is placed.
...
while(start >= 0):
    ad_analytics_api_url_all_creatives = 'https://api.linkedin.com/v2/adCreativesV2?q=search&start={}&count=10'.format(start)
    ...
    if ad_creative_elements_null_check:
        ...
        while (i<10):
            try:
                ...
                if (flag == 1):
                    ad_analytics_api_url_creative = 'https://api.linkedin.com/v2/adAnalyticsV2?q=analytics&dateRange.start.month={}&dateRange.start.day={}&dateRange.start.year={}&dateRange.end.month={}&dateRange.end.day={}&dateRange.end.year={}&timeGranularity=DAILY&pivot=CREATIVE&creatives[0]=urn:li:sponsoredCreative:{}&count=1000'.format(start_date.month,start_date.day,start_date.year,e_d.month,e_d.day,e_d.year,creative_id_extracted)  
                elif (flag == 2):
                    ad_analytics_api_url_creative = 'https://api.linkedin.com/v2/adAnalyticsV2?q=analytics&dateRange.start.month={}&dateRange.start.day={}&dateRange.start.year={}&timeGranularity=DAILY&pivot=CREATIVE&creatives[0]=urn:li:sponsoredCreative:{}&count=1000'.format(start_date.month,start_date.day,start_date.year,creative_id_extracted)  
                else:
                    ad_analytics_api_url_creative = 'https://api.linkedin.com/v2/adAnalyticsV2?q=analytics&dateRange.start.month={}&dateRange.start.day={}&dateRange.start.year={}&dateRange.end.month={}&dateRange.end.day={}&dateRange.end.year={}&timeGranularity=DAILY&pivot=CREATIVE&creatives[0]=urn:li:sponsoredCreative:{}&count=1000'.format(start_date.month,start_date.day,start_date.year,e_d.month,e_d.day,e_d.year,creative_id_extracted)  
...
#                            print ("Start Year======================:" ,data['elements'][element]['dateRange']['start']['year'] )
                            element+=1
                            date_attributes = datetime.date(creative_attributes['elements'][element]['dateRange']['start']['year'],creative_attributes['elements'][element]['dateRange']['start']['month'],creative_attributes['elements'][element]['dateRange']['start']['day'])#Year,Month,Day
                            print ("Date Attributes",date_attributes)
                            linked_in_attributes_list = [date_attributes,creative_attributes['elements'][element]['dateRange']['start']['day'],creative_attributes['elements'][element]['dateRange']['start']['month'], creative_attributes['elements'][element]['dateRange']['start']['year'],creative_id_extracted, creative_name, campaign_id_extracted[0], campaign_name, account_id,
                       account_name,
                       destination_url,
                       cid,creative_url_shares_id[0],creative_attributes['elements'][element]['clicks'],creative_attributes['elements'][element]['impressions'],creative_attributes['elements'][element]['costInUsd']]
...
with open(output_file_name, "w") as f:
    writer = csv.writer(f)
    writer.writerow(["Date", "start_day", "start_month","start_year","li_ad_creative_id","li_ad_creative_name","li_ad_campaign_id","li_ad_campaign_name","li_ad_account_id","li_ad_account_name","destination_url","li_sfdc_campaign_id","li_creative_shares_url_id","clicks","impressions","amount_spend_in_usd"])
